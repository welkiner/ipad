...
-(HETTextViewAgent *)agent{
    return objc_getAssociatedObject(self, @selector(setAgent:));
}
-(void)setAgent:(HETTextViewAgent *)agent{
...
- (void)het_setText:(NSString *)text {
    ...
    if (!self.agent) {
        self.agent = [HETTextViewAgent new];
    }
